...
with meta_input:
    st.session_state.task = st.sidebar.selectbox('选择任务', ['主旋律', '发展旋律', '结构', '和弦', '后期'])

    st.sidebar.header('参数选择') 

    st.session_state.time_signature = st.sidebar.selectbox('选择拍号', ['2/4', '3/4', '4/4'],
                                on_change=modify_mc)
    st.session_state.key_signature = st.sidebar.selectbox('选择调号', ['C大调', 'G大调', 'D大调', 'A大调', 'E大调'],
                                on_change=modify_mc)
